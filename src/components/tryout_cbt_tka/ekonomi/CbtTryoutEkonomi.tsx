...
export default function CbtTryoutEkonomi({ onBack }: CbtTryoutEkonomiProps) {
  ...
  const evaluation = useMemo(() => {
    ...
    const report: CbtReportData = {
      title: 'Try Out CBT TKA: Ekonomi (Pusmendik)',
      subject: 'Ekonomi',
      score: scaledScore,
      ...
      targetProdi: 'Ilmu Ekonomi, Akuntansi & Manajemen Bisnis',
      keketatan: 'Keketatan Sangat Kompetitif (Top 3%)',
      strongSubjects: Array.from(strongTopics),
      weakSubjects: Array.from(weakTopics),
      radarScores: {
        'Mikroekonomi & Permintaan Pasar': Math.min(100, Math.round(correct * 5.2)),
        'Makroekonomi & Kebijakan Moneter': Math.min(100, Math.round(correct * 5.0 + 5)),
        'Akuntansi & Persamaan Dasar': Math.min(100, Math.round(correct * 4.9 + 8)),
        'Perdagangan Internasional': Math.min(100, Math.round(correct * 5.1 + 4)),
        'Pembangunan & Ketenagakerjaan': Math.min(100, Math.round(correct * 4.8 + 6))
      }
    };
  ...
  return (
    <div className={`min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans ${isFullscreen ? 'p-0' : 'p-2 sm:p-4'}`}>
      {/* Top Header */}
      <header className="bg-slate-800/90 backdrop-blur-md border border-slate-700/80 rounded-xl px-4 py-3 mb-3 flex flex-wrap items-center justify-between gap-3 shadow-lg">
        <div className="flex items-center gap-3">
          {onBack && (
            <button
              onClick={onBack}
              className="p-2 rounded-lg bg-slate-700/60 hover:bg-slate-700 text-slate-300 hover:text-white transition-all flex items-center gap-1 text-sm font-medium"
        ...
        <div className="flex items-center gap-3">
          {!submitted && (
            <div className={`flex items-center gap-2 px-3.5 py-1.5 rounded-lg font-mono font-bold text-sm shadow-inner border ${
              timeLeft < 300
          ...
          <button
            ...
            className="p-2 rounded-lg bg-slate-700/60 hover:bg-slate-700 text-slate-300 hover:text-white transition-all hidden sm:flex items-center justify-center"
          ...
          {submitted && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-sm font-semibold rounded-lg shadow-lg hover:shadow-emerald-500/25 transition-all transform active:scale-95"
            >
              <BookOpen className="w-4 h-4" />
              <span>Buka Jendela Pembahasan</span>
            </button>
          )}
        </div>
      </header>
      ...
      {!submitted ? (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 flex-1">
          ...
          <div className="lg:col-span-8 flex flex-col bg-slate-800/80 backdrop-blur border border-slate-700/80 rounded-xl p-4 sm:p-6 shadow-xl relative min-h-[500px]">
            {/* Question Sub-header */}
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-700/70">
              <div className="flex items-center gap-2">
                <span className="px-3 py-1 bg-amber-500/20 text-amber-300 border border-amber-500/40 rounded-lg text-sm font-bold font-mono">
            ...
            {currentQ.stimulus && (
              <div className="mb-5 p-4 rounded-xl bg-slate-900/80 border border-slate-700/70 text-slate-200 text-sm leading-relaxed overflow-x-auto shadow-inner">
                <div className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <Coins className="w-3.5 h-3.5" />
                  <span>Teks Stimulus / Informasi / Data Ekonomi:</span>
                </div>
            ...
            {/* Question Text */}
            <div className="mb-6 text-slate-100 text-base font-medium leading-relaxed">
            ...
            <div className="flex-1 space-y-3 mb-6">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-3.5 rounded-xl border transition-all flex items-start gap-3.5 group ${
                          isSelected
                            ? 'bg-amber-600/25 border-amber-500 text-white shadow-md shadow-amber-500/10'
                            : 'bg-slate-800/60 border-slate-700/80 text-slate-300 hover:bg-slate-700/50 hover:border-slate-600'
                        }`}
                      >
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs uppercase shrink-0 transition-colors ${
                            isSelected
                        ...
                        <div className="text-sm font-normal pt-0.5 leading-relaxed flex-1">
              ...
              {/* Type 2: Multiple Checkboxes */}
              {currentQ.type === 'checkboxes' && currentQ.options && (
                <div className="space-y-2.5">
                  <p className="text-xs text-amber-400 font-semibold mb-1">
                    * Pilihlah semua opsi jawaban yang benar (bisa lebih dari satu):
                  </p>
                  {currentQ.options.map(opt => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-6 h-6 rounded-md flex items-center justify-center font-bold text-xs shrink-0 transition-colors border ${
                            isChecked
                              ? 'bg-amber-500 border-amber-400 text-slate-950'
                              : 'bg-slate-700/80 border-slate-600 text-transparent group-hover:border-slate-500'
                          }`}
                        >
              ...
              {/* Type 3: True / False Statements Table */}
              {currentQ.type === 'true-false-table' && currentQ.statements && (
                <div className="overflow-x-auto rounded-xl border border-slate-700 bg-slate-900/60">
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-800/90 text-slate-300 text-xs font-semibold uppercase border-b border-slate-700">
                      <tr>
                        <th className="p-3.5 w-12 text-center">No</th>
                        <th className="p-3.5">Pernyataan / Analisis Transaksi</th>
                    ...
                    <tbody className="divide-y divide-slate-800 text-slate-200">
                      {currentQ.statements.map((st, sIdx) => {
                        const stAnswer = answers[currentQ.id]?.[st.id];
                        return (
                          <tr key={st.id} className="hover:bg-slate-800/40 transition-colors">
                            <td className="p-3.5 text-center font-mono text-xs text-slate-400 font-semibold">
                              {sIdx + 1}
                            </td>
                            <td className="p-3.5 text-sm leading-relaxed">
                            ...
                            <td className="p-3.5 text-center">
                              <button
                                ...
                                className={`w-8 h-8 rounded-lg font-bold text-xs transition-all border ${
                                  stAnswer === true
                                    ? 'bg-emerald-500 border-emerald-400 text-slate-950 font-black ring-2 ring-emerald-400/40'
                            ...
                            <td className="p-3.5 text-center">
                              <button
                                ...
                                className={`w-8 h-8 rounded-lg font-bold text-xs transition-all border ${
                                  stAnswer === false
                                    ? 'bg-rose-500 border-rose-400 text-white font-black ring-2 ring-rose-400/40'
                                    : 'bg-slate-800 border-slate-700 text-slate-400 hover:border-slate-500'
                                }`}
                              >
                                S
            ...
            <div className="flex items-center justify-between pt-4 border-t border-slate-700/70 mt-auto">
              <button
                onClick={handlePrev}
                disabled={currentIdx === 0}
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-700/80 hover:bg-slate-700 text-slate-200 text-sm font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition-all"
              >
              ...
              {currentIdx < ekonomiTryoutData.length - 1 ? (
                <button
                  onClick={handleNext}
                  className="flex items-center gap-1.5 px-5 py-2 rounded-lg bg-amber-600 hover:bg-amber-500 text-slate-950 font-bold text-sm shadow-md hover:shadow-amber-500/20 transition-all"
                >
                ...
                <button
                  onClick={() => {
                    if (window.confirm('Apakah Anda yakin ingin menyelesaikan dan mengumpulkan try out ini?')) {
                      setSubmitted(true);
                    }
                  }}
                  className="flex items-center gap-1.5 px-5 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm shadow-lg hover:shadow-emerald-500/25 transition-all"
                >
          ...
          <div className="lg:col-span-4 flex flex-col gap-4">
            <div className="bg-slate-800/80 backdrop-blur border border-slate-700/80 rounded-xl p-4 sm:p-5 shadow-xl">
              ...
              <div className="grid grid-cols-5 gap-2 mb-4">
                {ekonomiTryoutData.map((quest, idx) => {
                  ...
                  if (isAnswered) {
                    bgClass = 'bg-amber-600 border-amber-500 text-slate-950 font-bold shadow-sm';
                  }
                  if (isFlag) {
                  ...
                  return (
                    <button
                      ...
                      className={`h-10 rounded-lg text-xs font-bold border transition-all flex flex-col items-center justify-center relative ${bgClass}`}
                    >
              ...
              <div className="mt-5">
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-gradient-to-r from-amber-600 to-yellow-600 hover:from-amber-500 hover:to-yellow-500 text-slate-950 font-black rounded-lg shadow-lg hover:shadow-amber-500/25 transition-all text-sm flex items-center justify-center gap-2"
                >
                  <CheckCircle2 className="w-4 h-4" />
        ...
        /* Report & Analysis View when submitted */
        <div className="space-y-6">
          <div className="bg-slate-800/90 border border-slate-700 rounded-xl p-6 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="p-4 bg-amber-500/20 text-amber-400 rounded-2xl border border-amber-500/30">
                <Award className="w-10 h-10" />
              </div>
              <div>
                <h2 className="text-xl font-black text-white">Hasil Try Out CBT: TKA Ekonomi</h2>
                <p className="text-sm text-slate-400">
                  Ujian telah selesai dinilai dengan metode standar Pusmendik & SNPMB
                </p>
                <div className="flex flex-wrap gap-2 mt-2">
                  <span className="px-2.5 py-1 rounded bg-emerald-500/20 text-emerald-300 text-xs font-semibold border border-emerald-500/30">
                    Benar: {evaluation.correct} Soal
                  </span>
                  <span className="px-2.5 py-1 rounded bg-rose-500/20 text-rose-300 text-xs font-semibold border border-rose-500/30">
                    Salah / Kosong: {evaluation.wrong} Soal
                  </span>
                  <span className="px-2.5 py-1 rounded bg-amber-500/20 text-amber-300 text-xs font-semibold border border-amber-500/30 font-mono">
                    Skor TKA: {evaluation.scaledScore} / 1000
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-5 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-sm shadow-lg hover:shadow-emerald-500/25 transition-all flex items-center gap-2"
              >
              ...
              <button
                ...
                className="p-3 rounded-xl bg-slate-700 hover:bg-slate-600 text-slate-200 transition-all"
                title="Ulangi Try Out"
              >
      ...
      {/* ========================================================================= */}
      {/* SEPARATE DISCUSSION WINDOW (Jendela Pembahasan Terpisah dari Soal) */}
      {/* ========================================================================= */}
          ...
          <div className="bg-slate-900 border border-slate-700/80 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <div className="bg-slate-800 px-5 py-4 border-b border-slate-700 flex items-center justify-between gap-4">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-slate-700/60 hover:bg-rose-600/80 text-slate-300 hover:text-white transition-all"
                ...
                {filteredPembahasanQuestions.map((fq, fIdx) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-7 h-7 rounded-md font-bold text-xs transition-all flex items-center justify-center shrink-0 border ${
                        isCurrent
            ...
            <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-6">
              ...
              <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4">
                ...
                {activePembahasanQ.stimulus && (
                  <div className="mb-4 p-3.5 bg-slate-900/80 rounded-lg border border-slate-700/60 text-sm text-slate-300 leading-relaxed overflow-x-auto">
                ...
                <div className={`p-4 rounded-xl border ${
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`text-xs font-bold px-2 py-0.5 rounded ${
                      ...
                      {evaluation.questionResults[activePembahasanQ.id]?.isCorrect ? 'BENAR' : 'SALAH / TIDAK LENGKAP'}
                    </span>
                  </div>
                </div>
              ...
              <div className="bg-slate-800/90 border border-slate-700/90 rounded-xl p-5 shadow-lg space-y-3">
                <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm border-b border-slate-700 pb-2">
                  <BookOpen className="w-4 h-4" />
                  <span>Pembahasan Runtut & Mendalam (LaTeX):</span>
                </div>
            ...
            <div className="bg-slate-800 px-5 py-3 border-t border-slate-700 flex items-center justify-between gap-3">
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-slate-200 text-xs font-semibold disabled:opacity-40 disabled:cursor-not-allowed transition-all"
              >
                <ChevronLeft className="w-4 h-4" />
